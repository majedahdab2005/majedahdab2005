...
`assets/metrics.svg` is generated by the **refresh metrics card** workflow, which runs
daily and reads your public contribution numbers through the GitHub API. Nothing
third party is involved, so it cannot break when someone else's service goes down.
Run it once by hand from the Actions tab if you want it refreshed immediately.

## 5. Make the numbers count your Kagu work
...
## 6. Finishing touches on the profile itself
...
| Metrics card layout | `scripts/build_metrics.py` |
